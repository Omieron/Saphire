            ...
            {showModal && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
                        <div className="p-6 border-b border-[var(--color-border)] flex items-center justify-between bg-[var(--color-bg)] rounded-t-2xl">
                            <h3 className="text-xl font-bold">{editItem ? t.tasks.editTask : t.tasks.addTask}</h3>
                            <button onClick={() => setShowModal(false)} className="p-2 hover:bg-slate-200 rounded-lg transition-colors"><XCircle size={20} /></button>
                        </div>
                        <form onSubmit={handleSubmit} className="p-6 space-y-6">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-[var(--color-bg)] rounded-xl p-4 border border-[var(--color-border)]">
                                    <div className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider mb-3">{t.tasks.selectTemplate}</div>
                                ...
                                <div className="bg-[var(--color-bg)] rounded-xl p-4 border border-[var(--color-border)]">
                                    <div className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider mb-3">{t.tasks.assignmentType}</div>
                                    <div className="flex bg-[var(--color-surface)] p-1 rounded-lg border border-[var(--color-border)]">
                                        <button type="button" onClick={() => setFormData({ ...formData, type: 'RECURRING' })}
                                            className={`flex-1 py-1.5 px-3 rounded-md text-sm font-medium transition-all ${formData.type === 'RECURRING' ? 'bg-teal-500 text-white shadow-sm' : 'text-[var(--color-text-secondary)] hover:text-[var(--color-text)]'}`}>
                                            {t.tasks.recurring}
                                        </button>
                                        <button type="button" onClick={() => setFormData({ ...formData, type: 'ONCE' })}
                                            className={`flex-1 py-1.5 px-3 rounded-md text-sm font-medium transition-all ${formData.type === 'ONCE' ? 'bg-teal-500 text-white shadow-sm' : 'text-[var(--color-text-secondary)] hover:text-[var(--color-text)]'}`}>
                                            {t.tasks.once}
                                        </button>
                                    </div>
                                </div>
                            ...
                            {formData.templateId > 0 && templates.find(t => String(t.id) === String(formData.templateId))?.contextType === 'MACHINE' && (
                                <div className="bg-[var(--color-bg)] rounded-xl p-4 border border-[var(--color-border)]">
                                    <div className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider mb-3">{t.qcTemplates.selectMachine}</div>
                            ...
                            {formData.templateId > 0 && templates.find(t => String(t.id) === String(formData.templateId))?.contextType === 'PRODUCT' && (
                                <div className="bg-[var(--color-bg)] rounded-xl p-4 border border-[var(--color-border)]">
                                    <div className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider mb-3">{t.qcTemplates.selectProduct}</div>
                            ...
                            <div className="bg-[var(--color-bg)] rounded-xl p-4 border border-[var(--color-border)]">
                                <div className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider mb-3">{t.tasks.assignToUsers}</div>
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-40 overflow-y-auto p-3 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg">
                                    {users.filter(u => u.role.includes('OPERATOR')).length === 0 ? (
                                        <div className="col-span-full py-4 text-center text-xs text-[var(--color-text-secondary)] italic">
                                            Role: OPERATOR olan kullanıcı bulunamadı
                                        </div>
                                    ) : (
                                        users.filter(u => u.role.includes('OPERATOR')).map(user => (
                                            <label key={user.id} className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer border transition-all ${formData.userIds.some(id => id == user.id) ? 'border-teal-500 bg-teal-500/10' : 'border-[var(--color-border)] hover:bg-[var(--color-surface-hover)]'}`}>
                                                <input type="checkbox" checked={formData.userIds.some(id => id == user.id)} onChange={() => toggleUser(user.id)} className="sr-only" />
                                                <div className="flex-1 truncate">
                                                    <div className={`text-xs font-semibold ${formData.userIds.some(id => id == user.id) ? 'text-teal-500' : 'text-[var(--color-text)]'}`}>{user.fullName}</div>
                                                    <div className="text-[10px] text-[var(--color-text-secondary)]">@{user.username}</div>
                                                </div>
                                                {formData.userIds.some(id => id == user.id) && <CheckCircle size={14} className="text-teal-500" />}
                                            </label>
                                        ))
                                    )}
                                </div>
                            ...
                            <div className="bg-[var(--color-bg)] rounded-xl p-4 border border-[var(--color-border)]">
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    <button type="button" onClick={handleAddSchedule} className="text-xs text-teal-500 hover:text-teal-600 font-semibold flex items-center gap-1">
                                        <Plus size={14} /> {t.tasks.addSchedule}
                                    </button>
                                </div>
                                <div className="space-y-2">
                                    {formData.schedules.map((schedule, idx) => (
                                        <div key={idx} className="flex items-end gap-3 p-3 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl group relative">
                                            {formData.type === 'RECURRING' ? (
                                                <div className="flex-1">
                                                    <label className="block text-[10px] uppercase font-bold text-[var(--color-text-secondary)] mb-1">{t.tasks.dayOfWeek}</label>
                                                    <select value={schedule.dayOfWeek} onChange={(e) => updateSchedule(idx, 'dayOfWeek', Number(e.target.value))}
                                                        className="w-full bg-[var(--color-surface)] dark:bg-[var(--color-bg)] border border-[var(--color-border)] rounded px-2 py-1.5 text-sm outline-none text-[var(--color-text)]">
                                                        {[1, 2, 3, 4, 5, 6, 7].map(d => <option key={d} value={d} className="bg-[var(--color-surface)] text-[var(--color-text)]">{(t.tasks.days as any)[d]}</option>)}
                                                    </select>
                                                </div>
                                            ) : (
                                                <div className="flex-1">
                                                    <label className="block text-[10px] uppercase font-bold text-[var(--color-text-secondary)] mb-1">{t.tasks.specificDate}</label>
                                                    <input type="date" value={schedule.specificDate} onChange={(e) => updateSchedule(idx, 'specificDate', e.target.value)}
                                                        className="w-full bg-[var(--color-surface)] dark:bg-[var(--color-bg)] border border-[var(--color-border)] rounded px-2 py-1.5 text-sm outline-none text-[var(--color-text)] [color-scheme:light] dark:[color-scheme:dark]" />
                                                </div>
                                            )}
                                            <div className="w-32">
                                                <label className="block text-[10px] uppercase font-bold text-[var(--color-text-secondary)] mb-1">{t.tasks.startTime}</label>
                                                <input type="time" value={schedule.startTime} onChange={(e) => updateSchedule(idx, 'startTime', e.target.value)}
                                                    className="w-full bg-[var(--color-surface)] dark:bg-[var(--color-bg)] border border-[var(--color-border)] rounded px-2 py-1.5 text-sm outline-none text-[var(--color-text)] [color-scheme:light] dark:[color-scheme:dark]" />
                                            </div>
                                            <div className="w-32">
                                                <label className="block text-[10px] uppercase font-bold text-[var(--color-text-secondary)] mb-1">{t.tasks.endTime}</label>
                            ...
                            <div className="flex gap-3 pt-4">
                                <button type="button" onClick={() => setShowModal(false)} className="flex-1 px-4 py-3 border border-[var(--color-border)] rounded-xl hover:bg-[var(--color-surface-hover)] text-[var(--color-text)] transition-colors font-medium">
                                    {t.common.cancel}
                                </button>
                                <button type="submit" disabled={saving} className="flex-1 px-4 py-3 bg-teal-500 text-white rounded-xl hover:bg-teal-600 transition-colors font-medium shadow-lg shadow-teal-500/20">
